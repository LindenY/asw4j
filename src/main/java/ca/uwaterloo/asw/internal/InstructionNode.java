...
public class InstructionNode {
	...
	public InstructionNode(
			String[] requiredDataNames,
			Class<?>[] requiredDataTypes, 
			String producedDataName,
		...
		for (int i=0; i<requiredDataNames.length; i++) {
			...
			Type type = requiredDataTypes.length <= i ? null : requiredDataTypes[i];
	...
	public static boolean getInstructionAsyncSupport(Class<? extends Instruction<?, ?>> instructionClass) {
...
/*	public static List<TypeToken<?>> getInstructionRequireDatas(
			Class<? extends Instruction<?, ?>> instructionClass) {
	...
	public static TypeToken<?> getInstructionProduceData(
			Class<? extends Instruction<?, ?>> instructionClass) {
	...
	public static final String[] getInstructionRequireDataNames(
		...
		if (requireDataAnnotation != null) {
			checkRequireDataArgumentsCondition(requireDataAnnotation, instructionClass);
	...
	private static final void checkRequireDataArgumentsCondition(
		...
		Type parameterizedType = getInstructionActualParameterizedArgumentAtIndex(instructionClass, 0);
		...
		if (names.length > 1 && types.length > 1 && names.length != types.length) {
			throw new IllegalArgumentException();
		}
